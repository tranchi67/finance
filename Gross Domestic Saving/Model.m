...
resubcost = resubLoss(t,'Subtrees','all'); [cost,secost,ntermnodes,bestlevel] = cvloss(t,'Subtrees','all'); plot(ntermnodes,cost,'b-', ntermnodes,resubcost,'r--')
figure(gcf);
xlabel('Number of terminal nodes'); ylabel('Cost (misclassification error)') legend('Cross-validation','Resubstitution')
...
plot(ntermnodes(bestlevel+1), cost(bestlevel+1), 'mo') legend('Cross-validation','Resubstitution','Min + 1 std. err.','Best choice') hold off
...
%Generating an exponentially spaced set of values from 10 through 100 %that represent the minimum number of observations per leaf node leafs = logspace(1,2,10);
%Creating cross-validated classification trees
%Specifying to grow each tree using a minimum leaf size in leafs rng('default')
N = err for
end
figure
plot(leafs,err);
xlabel('Min Leaf Size'); ylabel('cross-validated error');
%The best leaf size is 12 observations per leaf
%Comparing the near-optimal tree with 12 observations per leaf
%with the default tree, which uses 10 observations per parent node and 1 observation per leaf
DefaultTree = t;
OptimalTree = fitctree(Xtrain,Ytrain,'MinLeafSize',12); view(OptimalTree,'mode','graph')
%calcilating errors for comparison
resubOpt = resubLoss(OptimalTree);
lossOpt = kfoldLoss(crossval(OptimalTree)); resubDefault = resubLoss(DefaultTree); lossDefault = kfoldLoss(crossval(DefaultTree)); resubOpt,resubDefault,lossOpt,lossDefault
...
%Setting 'TreeSize' to 'SE' (default) to find the maximal pruning level %for which the tree error does not exceed the error from the best level %plus one standard deviation
numel(leafs);
= zeros(N,1);
n=1:N
t = fitctree(Xtrain,Ytrain,'CrossVal','On',...
'MinLeafSize',leafs(n)); err(n) = kfoldLoss(t);
22
...
%Setting 'TreeSize' to 'SE' (default) to find the maximal pruning level %for which the tree error does not exceed the error from the best level %plus one standard deviation
[~,~,~,bestlevel] = cvLoss(OptimalTree,'SubTrees','All') %best level = 13. Prunning the tree to the level 13. tree_opt = prune(OptimalTree,'Level',13); view(tree_opt,'Mode','Graph')
...
glm = fitglm(Xtrain,Ytrain,'linear','Distribution','binomial','link','logit') %not all variables have p-value < 0.5. Removing x3 for improving the model glm1 = removeTerms(glm,'x3')
plotSlice(glm)
%stepwise model
sw_glm = stepwiseglm(Xtrain,Ytrain,'linear','Distribution','binomial','link','logit') sw_glm1 = stepwiseglm(Xtrain,Ytrain,'constant','Distribution','binomial','upper','linea r')
%prediction for test set with glm
Y_glm = predict(glm,Xtest);
Y_glm = round(Y_glm);
% Compute the confusion matrix
C_glm = confusionmat(double(Ytest),Y_glm);
%prediction for test set with sw_glm
