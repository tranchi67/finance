...
[Pd,V,me,l] = pcaproj(sD_norm,2); Pm = pcaproj(sMap,V,me); % PC- projection
...
hold on som_grid(sMap,'Label',cellstr(int2str(hits)),...
'Line','none','Marker','none','Labelcolor','k'); hold off
title('Color code & hits')
% lables
subplot(1,2,2)
som_cplane(sMap,'none')
hold on som_grid(sMap,'Label',sMap.labels,'Labelsize',8,...
...
[c,p,err,ind] = kmeans_clusters(sMap, 7); % find at most n clusters. The optimal amount of clusters has the lowest k
                        % (you might want to increase n to allow more
clusters)
plot(1:length(ind),ind,'x-') [dummy,i] = min(ind);
cl = p{i};
title('Optimal # of clusters') % distance matrix subplot(1,3,2) som_cplane(sMap,Code,Dm) title('Distance matrix')
% clusters
    subplot(1,3,3)
    som_cplane(sMap,cl)
    title('Clusters')
...
for j=1:i
disp(strcat('cluster_',num2str(j))); % displaying cluster # csS_cl=som_stats(V{j}); % getting stats struct from data cluster for k=1:size(vnames,2) % this loop adds variable names
csS_cl{k}.name=vnames{k};
end
stats_cl=som_stats_table(csS_cl); % creating table of stats som_table_print(stats_cl); % printing table in command window disp('______________________________________________');
end
